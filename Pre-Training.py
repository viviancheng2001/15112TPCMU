...
def round_rectangle(canvas, x1, y1, x2, y2, r=25, **kwargs):
    ...
    y1 + r, x2, y2 - r, x2, y2 - r, x2, y2, x2 - r, y2, x2 - r, y2, x1 + r, y2,
    x1 + r, y2, x1, y2, x1, y2 - r, x1, y2 - r, x1, y1 + r, x1, y1 + r, x1, y1)
...
def init(data):
    ...
    data.questions = ["Experiments using the two mutant strains P and Q \n"
                      "reveal that strain P accumulates citrulline,but strain Q does not. \nWhich of the following statements is \nmost consistent with the data provided?"]
    data.answerChoices = ['1) Strain Q has only one mutation.\n '
                          '2) Strain P has a mutation in argF only. \n'
                          '3) Strain P has mutations in argF, argG and argH. \n'
                          '4) Strain P has a mutation in argG only.']
    data.correctAnswers = [4]
    ...
    if data.timer%10 == 0:
        ...
        if not frame.hands.is_empty:
            ...
            if not fingers.is_empty:
                numFingersExtended = data.fingerCounter.on_frame(data.controller)
...
def redrawAll(canvas, data):
    ...
    canvas.create_text(data.width/2, data.height/2 - 200,font = "Arial 20 bold",
